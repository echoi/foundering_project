...
we will consider contributions of the upper mantle heterogeneity to these stress fields and discuss the slip tendency of faults in the seismic zones based on the Coulomb failure criterion~\citep[e.g.,][]{king1994static, freed2005earthquake, li2007stress}. \citet{ghosh2019role} took a similar approach to explaining the intraplate seismicity in the CEUS but our study differs in the scale of heterogeneity investigated. \note[EC]{I shortened and move here your original paragraph on Gosh et al., 2019. If you want, a discussion on their findings can be included in the Discussion section.}\citet{ghosh2019role} invoked long-wavelength lateral variations in viscosity of the lithosphere which \annote[EC]{is dependent on the tectonic setting}{Clarify the meaning.}. In contrast, we consider short-wavelength viscosity contrasts originating from the high-resolution heterogeneities imaged in~\citet{Biryol_2016}\rq{}s tomography model. 
...
%
\note[EC]{Describe what Fig. 10 shows. Then, the following reasoning for possible reasons can be a new paragraph.}
...
\note[EC]{A better thesis sentence would state what S$_H$ you computed means or how it is compared with that of Levandowski 2018.} We compute the maximum horizontal stress directions (S$_H$) and faulting style for our tomography-based heterogeneous model, HT, in Fig.~\ref{sigma1} and compare them with the proposed S$_H$ and faulting style from a recent stress inversion study by~\citet{levandowski2018updated}. \annote[EC]{It is important to note}{Why is it important when his and your S$_{H}$ roughly match? I don\rq{}t see a reason to emphasize that you used only a \lq\lq{}limited\rq\rq{} data set.} that~\citet{levandowski2018updated} computed the stress map of the continental US using all the past focal mechanisms which differ from the fault geometries considered in this study in the Table~\ref{table_fault}, computed using limited focal mechanism data. The S$_H$ computed from our model roughly matches with the S$_H$ obtained by~\citet{levandowski2018updated} for all the seismic zones except the ETSZ.  Our computed NNW-SSE S$_H$ for the ETSZ differs from the NE-SW SH direction determined using focal mechanism solutions by~\citet{levandowski2018updated} and by~\citet{mazzotti2010state}. This is not surprising as faulting in the ETSZ may be strongly influenced by density anomalies in the lower crust inherited from past tectonic events~\citet{levandowski2018updated}, which are not accounted in this study.

The distribution of stress regime parameter, $R$,~\citep{delvaux1997paleostress,simpson1997quantifying}, \note[EC]{Delvaux et al. 1997 shows up incorrectly in the bibliography.} computed based on the model HT, shows that the dominant faulting styles are thrust for the NMSZ\_NE, and oblique-thrust for the GCSZ (Fig.~\ref{sigma1}). These stress regimes are consistent with the proposed faulting styles in~\citet{levandowski2018updated} for these zones, but differ from the selected studies in Table~\ref{table_fault} which suggests strike-slip for the GCSZ and the NMSZ\_NE.  
