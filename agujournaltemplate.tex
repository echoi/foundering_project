...
%\add[AS]{The differences in the optimal orientations at each seismic zone computed for the} HT$-$HM model and those taken from the observational studies are illustrated in Fig.~\ref{summary}.
...
%
The directions of maximum horizontal stress (S$_H$) computed from the model HT roughly matches those obtained by~\citet{levandowski2018updated} in all the seismic zones but the ETSZ (Fig.~\ref{sigma1}).  S$_{H}$ based on HT is NNW-SSE in the ETSZ differing from the NE-SW direction determined using focal mechanism solutions by~\citet{levandowski2018updated} and by~\citet{mazzotti2010state}. This is not surprising as faulting in the ETSZ may be strongly influenced by density anomalies in the lower crust inherited from past tectonic events~\citet{levandowski2018updated}, which are not accounted in this study.
...
\begin{figure}[ht]
    ...
    \caption{Velocity (arrows) and viscosity fields from the model HT on the slice AA' (marked in Fig \ref{fig_model}).}
%and the viscosity distribution computed from the temperatures based on the tomography by \citet{Biryol_2016}. 
%The extreme high-velocity vectors observed west of the NMSZ and out of the plane are from the upward return flow due to the downward pull of the lithospheric drip and are likely an artifact due to fixed boundary conditions at the sides.}
...
%    We discuss the stress implications of lithospheric foundering using instantaneous models with a drip. 
