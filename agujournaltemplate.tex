...
\begin{figure}[ht]
    ...
    \caption{\note[EC]{The figure title needs to be corrected, too.} Depth profile of the laterally-averaged P-wave velocity sensitivity to temperature. See text for the details of calculations.}
    \label{fig_temp}
 \end{figure}
...
\begin{figure}[ht]
    ...
    \caption{Model domain and cross-sections of the \annote[EC]{computed viscosity}{Does this mean the viscosity after the equilibrium is achieved?} based on the regional tomography by \citet{Biryol_2016}. Gray isosurface represents P-wave anomalies $>$ 2 \% in the region interpreted as lithospheric foundering. 
%Instantaneous flow along slice AA' passing through the ETSZ and NMSZ is discussed in Fig. \ref{velocity_pattern}. 
The state boundaries are drawn in solid black and red dots are epicenters for the earthquake catalog used in Fig. \ref{figone}. The inset on the left bottom shows the \add[EC]{reference?} temperature profile with depth in the crust. The boundary conditions are annotated on the top right.}
    \label{fig_model}
\end{figure}
...
(b) Molchan curves for dynamic topography ($h_{\text{dyn}}$), the rate of dynamic topography change ($dh_{dyn}/dt$) and gravitational potential energy (GPE) from the model HT.\note[EC]{Put the rate of dynamic topo before GPE in the legends.} (c) Molchan curves for change in differential stress ($\Delta \sigma_{\text{diff}}$), Coulomb stress change for the observed fault orientations at each seismic zone (Table~\ref{table_fault}) ($\Delta C$) and Coulomb stress change for the optimal fault geometry where the shear stress is maximum ($\Delta C_{\text{opt}}$). All are computed for HT$-$HM. (d) Same as (c) but for HT$-$HR.}
	\label{model_results}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Maximum horizontal stress directions (S$_H$) (gray lines) plotted on the map of the stress regime parameter (R) for the model HT. The white areas represent the regions where R is \annote[EC]{not interpolated}{Why not interpolated? If you mean that R values are not defined there, state so.}}
    \label{sigma1}
\end{figure}
