...
Static changes in Coulomb stress ($\Delta C$) of a model are defined as the Coulomb Failure Function (CFF) values minus those of another model~\citep{king1994static}\note[EC]{Exactly why is this reference cited? Did it first suggest to look at the static change in Coulomb stress?}:
\begin{equation}
...
\note[EC]{Although I'm afarid that it's just going back to your original idea, Please separate panels a and b, c and d into two or three separate figures. The current layout of the main result figure is awkward and inconvenient. Also, the first sentence doesn't convey much information. What do the changes in differential stress shown in this figure tell us? The thesis sentence should be an answer to this question.} Fig. \ref{model_results}(a) shows the computed change in differential stress for HT$-$HM (i.e., $\Delta|\sigma_1 - \sigma_3| = |\sigma_1 - \sigma_3|_{HT} - |\sigma_1 - \sigma_3|_{HM}$) at the seismogenic depth of 15 km for our model~\citep[e.g.,][]{mazzotti2010state}). Large positive values of $\Delta |\sigma_1 - \sigma_3|>15$ MPa are observed at the locations, $F_1$ and $F_2$. The seismic zones, ETSZ, SCSZ, GCSZ, CVSZ, and NMSZ, are overall correlated with positive $\Delta |\sigma_1 - \sigma_3|$ in the range of 2 to 10 MPa (Fig.\ref{model_results}b). On the other hand, differential stress changes in HT$-$HR show only a small positive values of 1 to 2 MPa in the horseshoe-shaped region surrounding the root including the ETSZ and NMSZ (\ref{model_results}b).
%
\begin{figure}[h!]
    \centering
