...
%
We compare the directions of maximum horizontal stress (S$_H$max) computed from the model HT at the depth of 15 km, which is the depth at which most earthquakes in this region occur~\citep{mazzotti2010state}, with the S$_H$max from the study by~\citet{levandowski2018updated}  (Fig.~\ref{sigma1}), who utilized focal mechanisms of the contiguous US to obtain their stresses. The S$_H$max from our model roughly match those obtained by~\citet{levandowski2018updated} in all the seismic zones but the ETSZ (Fig.~\ref{sigma1}). The angular deviation of our modeled S$_H$ values with the inverted S$_H$ results averaged over each seismic zone considered in Fig.~\ref{sigma1} is approximately 20$^\circ$, 68$^\circ$, 4$^\circ$, 12$^\circ$, and 15$^\circ$ for the NMSZ, ETSZ, GCSZ, CVSZ, and SCSZ, respectively.  S$_H$max based on HT is NNW-SSE in the ETSZ, differing from the NE-SW direction determined using focal mechanism solutions by~\citet{levandowski2018updated} and by~\citet{mazzotti2010state}. This misfit in the S$_H$max of the ETSZ has also been observed by~\citet{ghosh2019role} (Fig.7 of the paper) in their numerical models with lateral viscosity contrasts of the upper mantle. Our possible reasoning for this misfit is the stress concentration along the boundary of the strength contrast in the basement crustal rocks~\citep{powell1994seismotectonic}, which is difficult to resolve in a mantle tomography study. 
