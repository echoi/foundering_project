...
Our model setups are along similar lines with models proposed by~\citet{ghosh2019role} to explain the intraplate seismicity in the CEUS differing in the scale of heterogeneity investigated. The models suggested by~\citet{ghosh2019role} invoke long-wavelength variations in lateral viscosity of the lithosphere which is dependent on the tectonic setting. The models used in the study are computed directly from the high-resolution tomography study~\citep{Biryol_2016} and therefore, contains information of the short-wavelength viscosity contrasts within this region. 
...
We compute the maximum horizontal stress directions (S$_H$) and faulting style for our tomography-based heterogeneous model, HT, in Fig.~\ref{sigma1} and compare them with the proposed S$_H$ and faulting style from a recent stress inversion study by~\citet{levandowski2018updated}. \add[AS]{It is important to note that}~\citet{levandowski2018updated} compute the stress map of the continental US using all the past focal mechanisms which differ from the fault geometries considered in this study in the Table~\ref{table_fault}, computed using limited focal mechanism data. The S$_H$ computed from our model roughly matches with the S$_H$ obtained by~\citet{levandowski2018updated} for all the seismic zones except the ETSZ.  Our computed NNW-SSE S$_H$ for the ETSZ differs from the NE-SW SH direction determined using focal mechanism solutions by~\citet{levandowski2018updated} and by~\citet{mazzotti2010state}. This is not surprising as faulting in the ETSZ may be strongly influenced by density anomalies in the lower crust inherited from past tectonic events~\citet{levandowski2018updated}, which are not accounted in this study.
